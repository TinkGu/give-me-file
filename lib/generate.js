...
module.exports = function generate (name, src, dest, done) {
  ...
  if (opts.task && typeof opts.task.before === 'function') {
    opts.task.before(taskApi(taskApiArg))
  }

  metalsmith.use(askQuestions(opts.prompts))
    .use(filterFiles(opts.filters))
    .use(renderTemplateFiles(taskApiArg))
  ...
  metalsmith.clean(false)
    ...
    .build((err, files) => {
      ...
      if (typeof opts.task === 'function') {
        opts.task(taskApi(taskApiArg))
      } else if (opts.task && typeof opts.task.complete === 'function') {
        opts.task.complete(taskApi(taskApiArg))
      }
    })
...
function renderTemplateFiles (taskApiArg) {
  const opts = taskApiArg.meta
  const extraRenderData = taskApiArg.extraRenderData
  ...
  return (files, metalsmith, done) => {
    ...
    if (opts.task && typeof opts.task.afterAsk === 'function') {
      opts.task.afterAsk(taskApi(taskApiArg))
    }
